...
import image from "../../../assets/AA1wwv7n.jpg"
...
const NewsAndDetails = () => {
  ...
  const { insightsList, loading, error } = useSelector((state) => state.insights);
  ...
  const handleViewDetails = (item) => {
    navigate("/news", { state: item });
  };
  const handleViewInfoDetails = (item) => {
  ...
  const renderNewsFeed = () => (
    <div className="flex flex-row justify-between" >
       ...
       <div
          className="flex-1 p-4 w-full lg:w-2/3 h-fit"
            onScroll={onScroll}
            ref={listInnerRef}
      ...
      <aside className="w-full lg:w-1/3 p-2 bg-slate-200 hidden lg:block sticky top-0 h-fit">
        <h2 className="text-xl font-bold text-gray-800 mb-4">Latest Insights</h2>
        <div className="space-y-4">
          {insightsList?.map((update) => (
            <div
              key={update.id}
            ...
            >
              ...
              {isExpanded ? update.content : `${update.content.slice(0, 80)}...`}
                {update.content.length > 150 && (
                  <span className="text-blue-600 font-bold cursor-pointer" onClick={() => handleViewInfoDetails(update)}>Read more</span>
                )}
              </p>
        ...
        <div onClick={() => navigate("/insights")} className="bg-blue-700 w-full text-white text-sm font-medium px-6 py-2 rounded-md inline-block mt-3 text-center cursor-pointer">
          View More
        </div>
      </aside>
    </div>
  ...
  const renderFullNewsDetails = () => {
    ...
    return (
      <div >
        ...
        <div className="flex-1 p-4 w-full h-fit mt-16">
